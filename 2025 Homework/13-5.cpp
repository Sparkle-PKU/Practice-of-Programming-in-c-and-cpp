...
int main(void)
{
    int n;
    cin>>n;
    set<pair<int,int>>member;
    member.insert({1000000000,1});
    for(int i=0;i<n;i++)
    {
        int id,power;
        cin>>id>>power;
        auto it=member.lower_bound({power,0});
        pair<int,int> candidate;
        if(it==member.begin()) candidate=*it;
        else if (it==member.end()) candidate=*prev(it);
        else
        {
            
            auto after=*it;
            auto before=*prev(it);
            int diff1=abs(after.first-power);
            int diff2=abs(before.first-power);
            if(diff1<diff2) candidate=after;
            else if(diff2<diff1) candidate=before;
            else candidate=before.first<after.first?before:after;
            
        }
        cout<<id<<" "<<candidate.second<<"\n";
        member.insert({power,id});
    }
    return 0;
}
