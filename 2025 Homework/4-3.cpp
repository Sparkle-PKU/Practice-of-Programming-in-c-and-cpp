...
【关于武士特点】（第二版新增）
不同的武士有不同的特点。
（1）dragon 可以拥有一件武器。编号为n的dragon降生时即获得编号为 n%3 的武器。dragon还有“士气”这个属性，是个浮点数，其值为它降生后其司令部剩余生命元的数量除以造dragon所需的生命元数量。
（2）ninja可以拥有两件武器。编号为n的ninja降生时即获得编号为 n%3 和 (n+1)%3的武器。
（3）iceman有一件武器。编号为n的iceman降生时即获得编号为 n%3 的武器。
（4）lion 有“忠诚度”这个属性，其值等于它降生后其司令部剩余生命元的数目。
（5）wolf没特点。
请【注意】，在以后的题目里，武士的士气，生命值，忠诚度在其生存期间都可能发生变化，都有作用，武士手中的武器随着使用攻击力也会发生变化。
...
public:
    ...
    bool make_soldier()
    {
        for(int i=0;i<5;i++)
        {
            ...
            {
                int cost=soldier_hps[id];
                hp-=cost;
                total[names[id]]++;
                printf("%03d %s %s %d born with strength %d,%d %s in %s headquarter\n",t, name.c_str(), names[id].c_str(), index, cost,total[names[id]], names[id].c_str(), name.c_str());
                int num=index;
                ...
                {
                    int weapon_id=num%3;
                    double morale=static_cast<double>(hp)/cost;
                    printf("It has a %s,and it's morale is %.2f\n",weapons[weapon_id].c_str(),morale);
                }
                else if(id==1)//ninja
                {
                    int w1=num%3;
                    int w2=(num+1)%3;
                    printf("It has a %s and a %s\n",weapons[w1].c_str(),weapons[w2].c_str());
                }
                else if(id==2)//iceman
                {
                    int w=num%3;
...
int main()
{
    ...
    for(int r=1;r<=round;r++)
    {
        ...
        {
            bool red_made=false,blue_made=false;
            if(red_flag)
            {
                red_made=red.make_soldier();
                if(!red_made)
                {
                    red_flag=false;
                    if(!red_stop_printed)
            ...
            if(blue_flag)
            {
                blue_made=blue.make_soldier();
                if(!blue_made)
                {
                    blue_flag=false;
                    if(!blue_stop_printed)
                    {
